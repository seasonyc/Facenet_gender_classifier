...
def augmentate(image, level = 'simple'):
    if level == 'simple':
    ...
    r = tf.random.uniform([], 0.0, 1.0)
    image = tf.cond(greater(threshold, r),
        lambda:tf.contrib.image.rotate(image, (r - 0.05) * 3.14 * max_ang * 20 / 180 ),
        lambda:tf.identity(image))
...
'''
    consumer:
        ...
        translator
'''
def load_celeba(data_dir, batch_size, prefetch_batch=1, num_threads=4, buffer_size=4096, part='train', consumer = 'vae'):
    if consumer == 'vae':
        aug_level = 'simple'
    elif consumer == 'translator':
        aug_level = 'simple'
    elif consumer == 'classifier':
        aug_level = 'simple'
    ...
    def decode_and_preprocess_func(file, label=None):
        ...
        if consumer == 'classifier': 
            ...
            return (image, label, 1-label), (image, image, label, 1-label)
...
def test_load_celeba():
    with tf.Session() as sess:
        data, total_num = load_celeba('CelebA', 32, part='val', consumer = 'classifier')
        print(total_num)
